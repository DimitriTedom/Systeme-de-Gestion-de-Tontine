...
const IconContainer = memo(({ children, variant, className = '', theme = 'light' }: IconContainerProps) => (
  <motion.div
    ...
    className={cn(
      ...
      (!theme || theme === 'light') && "bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-950 dark:to-emerald-900 border border-emerald-200 dark:border-emerald-800 group-hover:shadow-xl group-hover:shadow-emerald-500/20 group-hover:border-emerald-300",
      className
    )}
  >
    <div className={cn(
      ...
      (!theme || theme === 'light') && "text-emerald-600 dark:text-emerald-400 group-hover:text-emerald-700 dark:group-hover:text-emerald-300"
    )}>
      {children}
    </div>
  </motion.div>
...
const Background = ({ theme = 'light' }: BackgroundProps) => (
  <div
    aria-hidden="true"
...
export const EmptyState = forwardRef<HTMLElement, EmptyStateProps>(({
  ...
  const getVariantClasses = (variant: string, theme: string) => {
    const variants: Record<string, Record<string, string>> = {
      default: {
        light: "bg-white dark:bg-neutral-900 border-dashed border-2 border-emerald-300 dark:border-emerald-700 hover:border-emerald-400 dark:hover:border-emerald-600 hover:bg-emerald-50/30 dark:hover:bg-emerald-950/30",
      ...
      subtle: {
        light: "bg-white dark:bg-neutral-900 border border-transparent hover:bg-emerald-50/20 dark:hover:bg-emerald-950/20",
      ...
      error: {
        light: "bg-white dark:bg-neutral-900 border border-red-200 dark:border-red-800 bg-red-50/50 dark:bg-red-950/50 hover:bg-red-50/80 dark:hover:bg-red-950/80",
  ...
  const getTextClasses = (type: 'title' | 'description', size: string, theme: string) => {
    const sizes: Record<'title' | 'description', Record<string, string>> = {
      title: {
        sm: "text-base",
        default: "text-lg",
        lg: "text-xl"
    ...
    const colors: Record<'title' | 'description', Record<string, string>> = {
      title: {
    ...
    return cn(sizes[type][size], colors[type][theme], type === 'title' ? "font-semibold" : "", "transition-colors duration-200");
  };
  ...
  const getButtonClasses = (size: string, theme: string) => {
    ...
    const themeClasses: Record<string, string> = {
      light: "bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white shadow-lg shadow-emerald-500/30 hover:shadow-emerald-500/50 border-0",
    ...
    return cn(
      "inline-flex items-center gap-2 border rounded-lg font-medium transition-all duration-200 relative overflow-hidden group/button disabled:opacity-50 disabled:cursor-not-allowed",
      sizeClasses[size],
      themeClasses[theme]
    );
  };
